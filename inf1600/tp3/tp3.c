...
static void matrix_row_aver(const int* inmatdata, int* outmatdata, int matorder);
...
int main(int argc, char** argv) {
   ...
   int* rowavervectorc;   /* The single row vector with row average values with the C function */
   int* rowavervectorasm; /* The single row vector with row average values with the assembler function */
   /* Program logic */
   printf("=========================\n");
   ...
   rowavervectorc = vector_create(matorder);
   matrix_row_aver(matdata, rowavervectorc, matorder);
   vector_print("average vector (C function)", rowavervectorc, matorder);
  ...
  // printf("asdaasd %d : \n", matrix_equals_asm(matdata,transposec , matorder)); 
   ...
   matrix_free(rowavervectorc);
   matrix_free(rowavervectorasm);
   return 0;
}
